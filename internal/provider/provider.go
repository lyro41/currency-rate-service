...
func GetCurrencyRate(client *http.Client, pair string) (api.RateStatus, time.Time, decimal.Decimal) {
	resp, err := client.Get(fmt.Sprintf("https://api.frankfurter.dev/v2/rate/%s", pair))
	if err != nil {
		fmt.Fprintf(os.Stderr, "get currency rate: %v\n", err)
		return api.StatusFailed, time.Now(), decimal.Decimal{}
	}
	...
	return api.StatusFetched, time.Now(), rate.Rate
}
