...
Операция выполняется асинхронно. При временных ошибках Frankfurter сервис автоматически выполняет до 3 попыток (по умолчанию) с экспоненциально увеличивающейся задержкой. Результат необходимо проверять по UUID: `pending` — запрос обрабатывается, `fetched` — курс получен, `failed` — курс получить не удалось.
